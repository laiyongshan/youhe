...
import com.example.youhe.youhecheguanjia.utils.EncryptUtils;
...
public class LoanActivity extends Activity {
    ...
    private void initLoan(String phoneNum, String userId) {
        ...
        Loan.newInstance(getApplicationContext())
    ...
    private void loanOpen(int loanType){
        if(loanType==1){//口袋借呗
            ...
            Loan.newInstance(getApplicationContext()).openSuDai(new LoanOpenListener() {
                @Override
                public void onOpenListener(int i, String s) {
                    switch (i) {
                        case 1:
