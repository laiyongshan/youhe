...
public final class CameraManager {
	...
	private static  int MIN_FRAME_HEIGHT = 400;
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			if (width < (mScreenWidth*5/9)) {
				width = (mScreenWidth*5/9);
			} else if (width > (mScreenWidth*5/9)) {
				width = (mScreenWidth*5/9);
			}
			int height = screenResolution.y * 3 / 4;
			if (height < (mScreenHeigh/3)) {
				height = (mScreenHeigh/3);
			} else if (height > (mScreenHeigh/3)) {
				height = (mScreenHeigh/3);
			}
	...
	public Rect getFramingRectInPreview() {
		if (framingRectInPreview == null) {
			...
			Point screenResolution = configManager.getScreenResolution();
			/*
			 * 横屏 rect.right = rect.right * cameraResolution.x /
